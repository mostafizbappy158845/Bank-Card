...
const CategoryComponent = ({ category, selected, cardSelected }) => {
  return (
    <div>
      ...
      <div className='main-part flex flex-col lg:flex-row '>
        <div className="left-seection w-4/5 grid grid-cols-1 h-52 md:grid-cols-2 gap-4 px-4">
          ...
          {category.filter((singleInfo) => singleInfo.category === selected)
            .map((singleInfo, i) => (
              ...
              bg-[#9EDDFF]">
                ...
                <div className='font-medium  lg:ml-8 '> <span className='text-red-500 font-bold uppercase text-xl'>MTB</span> & <span className='text-blue-800 font-bold uppercase text-xl'>{singleInfo.cardacategory}</span></div>
              </div>
