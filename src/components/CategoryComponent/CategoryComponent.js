  ...
  return (
    <div>
      <div className='mt-14 mx-4 flex flex-col md:flex-row justify-between'>
        <div>
          <p className=' texl-xl md:text-2xl lg:text-4xl font-mono md:font-semibold flex   mt-4 capitalize gap-4'> <span className='text-rose-500'>{category.category.type} </span></p>
      ...
      <div className='main-part flex flex-col lg:flex-row min-h-[500px]'>
        <div className="left-seection w-4/5 grid grid-cols-1 md:grid-cols-2 gap-4 px-4">
          ...
          {category.map((singleinfo,i) => (

            <div key={i} className="info-item flex flex-col lg:flex-row  border border-solid border-amber-500 rounded-md px-3
                          bg-[#9EDDFF]">
              <div className='text-xl md:text-2xl lg:text-4xl font-serif font-semibold md:font-bold lg:tracking-wide pt-14 mr-2 md:mr-4 lg:mr-8 uppercase'>{singleinfo.nameundertype} <br /> Hotel</div>
